...
class Package(object):
    ...
    def makeHpkg(self, requiresUpdater):
        ...
        output = check_output([Configuration.getPackageCommand(
        ), 'create', packageFile], cwd=self.packagingDir).decode('utf-8')
        info(output)
        # policy check
    ...
    def createBuildPackage(self):
        ...
        try:
            ...
            raise Exception('failure creating the build package: '
                            + exception.output.decode('utf-8')[:-1])
        info(output)
