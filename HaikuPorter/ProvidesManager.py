...
class ProvidesManager(object):
    ...
    def addProvidesFromPackage(self, package):
        for providesString in package.recipeKeys['PROVIDES']:
            self._addPackageProvidesInfo(
                package.revisionedName, providesString)
    ...
    def getMatchingProvides(self, resolvableExpression, anyHpkg=False,
        ...
        for provides in providesList:
            provideIsHpkg = (
                provides.packageInfo.path.endswith('.hpkg') if isinstance(
            ...
            if not matches:
                continue
            if (provides.compatibleVersion and versionCompare(
                    provides.compatibleVersion, version) > 0):
                continue
            if not updateDependencies and not missingDependencies:
                return provides
            if (found is None or
