...
class Source(object):
    ...
    def reset(self):
        ...
        output = check_output(
            ['git', 'reset', '--hard', 'ORIGIN'], cwd=self.sourceDir).decode('utf-8')
        info(output)
    ...
    def _initImplicitGitRepo(self):
        ...
        info(check_output(['git',
                           'symbolic-ref',
                           'HEAD',
                           'refs/heads/haikuport'],
                          cwd=self.sourceDir).decode('utf-8'))
        info(check_output(['git', 'add', '-f', '.'],
                          cwd=self.sourceDir).decode('utf-8'))
        info(check_output(['git', 'commit', '-m', 'import', '-q'],
                          cwd=self.sourceDir, env=self.gitEnv).decode('utf-8'))
        info(check_output(['git', 'tag', 'ORIGIN'],
                          cwd=self.sourceDir).decode('utf-8'))
