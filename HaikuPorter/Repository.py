...
class Repository(object):
        ...
        for portName in portNames:
            if not regExp or reSearch.search(portName):
                if returnPortNameVersions:
                    portNameVersion = self._portNameVersionForPortName(
                        portName)
    ...
    def _initAllPorts(self):
        ...
        # are supported, but we won't know until we have parsed the recipe
        # file.
        ...
        for portName in list(self._portVersionsByName.keys()):
            ...
            # It should likely be replaced by a proper manual refactoring of
            # versionCompare
            self._portVersionsByName[portName].sort(
                key=functools.cmp_to_key(versionCompare))
    ...
    def _removeStalePortForPackageMappings(self, activePorts):
        ...
        for packageName, portName in list(
                self._portNameForPackageName.items()):
            if portName not in self._portVersionsByName:
    ...
    def _partiallyExtractSourcePackageIfNeeded(self, sourcePackageName):
        ...
        if (not os.path.exists(recipeFilePath)
            ...
            entries = check_output([Configuration.getPackageCommand(
            ), 'list', '-p', sourcePackagePath]).decode('utf-8').splitlines()
            entries = [
