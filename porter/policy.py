...
class Policy(object):
        ...
        if rpath is not None:
            for rpath1 in rpath.split(':'):
                if rpath1.find('/.self/') != -1:
                    rpathDir = os.path.join(self.package.packagingDir, rpath1[rpath1.find(
                        '/.self/') + len('/.self/'):] + '/' + library)
                    if os.path.exists(rpathDir):
                        return False
                elif rpath1.find('$ORIGIN') != -1:
                    rpathDir = os.path.join(dirPath, rpath1[rpath1.find(
                        '$ORIGIN/') + len('$ORIGIN/'):] + '/' + library)
                    if os.path.exists(rpathDir):
                        return False

        # not provided by the package -- check whether it is required
        # explicitly
    ...
    def _checkMisplacedDevelopLibraries(self):
        ...
        for entry in os.listdir(libDir):
            ...
            path = libDir + '/' + entry
            self._violation(
                'development library entry "%s" should be placed '
                'in "develop/lib%s"' %
    ...
    def _checkGlobalWritableFilesRecursively(self, globalWritableFiles,
        ...
        if path in globalWritableFiles:
            if not globalWritableFiles[path]:
                self._violation(
                    'Included "%s" declared as not included global '
                    'writable %s' %
                    (path, fileTypes[path]))
            return
