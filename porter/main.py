...
class Main(object):
    ...
    def run(self, args):
        ...
        if self.options.repositoryUpdate \
            ...
            if self.options.checkRepositoryConsistency:
                self.repository.checkRepositoryConsistency(
        ...
        self._createRepositoryIfNeeded(
            self.options.quiet, self.options.verbose)
    ...
    def _listDependencies(self, port):
        ...
        presentDependencyPackageNames = [os.path.basename(
            package) for package in presentDependencyPackages]
    ...
    def _checkSourceTree(self, portArgument):
        if portArgument:
            ...
            portVersionsByName = self.repository.portVersionsByName
            somethingFailed = False
            for portName in sorted(
                    list(
                        portVersionsByName.keys()),
                    key=str.lower):
            ...
            if recipeFormatVersionMatch:
                try:
                    recipeFormatVersion = int(
                        recipeFormatVersionMatch.group(1))
                except ValueError:
                    pass
