...
class RemoteBuilder(object):
    def __init__(self, configFilePath, packagesPath, outputBaseDir,
        ...
        self.buildLogger = logging.getLogger(
            'builders.' + self.name + '.build')
    ...
    def _loadConfig(self, configFilePath):
        ...
        if 'portstree' not in self.config:
            raise Exception(
                'missing portstree config for builder ' +
                self.name)
    ...
    def _connect(self):
        try:
            ...
            # avoid DoSing the remote host, increasing delay as retries
            # increase.
...
class BuildMaster(object):
    def __init__(self, portsTreePath, packagesPath, options):
        ...
        if self.buildOutputBaseDir:
            ...
            self.buildOutputBaseDir = os.path.join(
                self.masterBaseDir, 'output')
    ...
    def schedule(self, port, requiredPackageIDs, presentDependencyPackages):
        ...
        for package in port.packages:
            ...
            self.addSkipped(
                port, 'some packages already exist, revision bump required')
            return
    ...
    def runBuilds(self, stdscr=None):
        ...
        try:
            ...
            self.totalBuildCount = len(
                self.scheduledBuilds) + len(self.blockedBuilds)
    ...
    def _fillPortsTreeInfo(self):
        try:
            ...
            origin = subprocess.check_output(['git',
                                              'remote',
                                              'get-url',
                                              'origin'],
                                             cwd=self.portsTreePath,
                                             stderr=subprocess.STDOUT).decode('utf-8')
            head = subprocess.check_output(['git',
                                            'rev-parse',
                                            'HEAD'],
                                           cwd=self.portsTreePath,
                                           stderr=subprocess.STDOUT).decode('utf-8')
            ...
            with self.buildableCondition:
                if len(self.scheduledBuilds) > 0:
                    ...
                    if self.buildStatus != 'waiting for packages':
                        self.logger.info(
                            'nothing buildable, waiting for packages')
