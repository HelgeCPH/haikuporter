...
class BuildPlatformHaiku(BuildPlatform):
    ...
    def init(self, treePath, outputDirectory, packagesPath,
        ...
        for entry in os.listdir(packagesDir):
            if (entry == 'haiku.hpkg' or (entry.startswith(
                    'haiku-') and entry.endswith('.hpkg'))):
    ...
    def findDirectory(self, which):
        ...
        if which not in self.findDirectoryCache:
            self.findDirectoryCache[which] = check_output(
                ['/bin/finddir', which]).decode('utf-8').rstrip()  # drop newline
    ...
    def activateBuildPackage(self, workDir, packagePath, revisionedName):
        ...
        packagesDir = buildPlatform.findDirectory(
...
class BuildPlatformUnix(BuildPlatform):
    ...
    def activateBuildPackage(self, workDir, packagePath, revisionedName):
        return self._activatePackage(
            packagePath, self._getPackageInstallRoot(
                workDir, packagePath), None, True)
    ...
    def setupNonChrootBuildEnvironment(self, workDir, secondaryArchitecture,
        ...
        print(
            'Activating haiku_cross_devel_sysroot package: ' +
            crossDevelPackage)
        ...
        for package in requiredPackages:
            self._activatePackage(
                package, self._getPackageInstallRoot(
                    workDir, package), '/boot/system')
